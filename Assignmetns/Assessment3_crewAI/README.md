Solution : Steps for execution : 
Step1 : created virtual environment and activated
Step2 : poetry install (changes python version as my current python version is different)
Step3 : poetry run python src/stock_analysis/main.py

Debugging : - Changed Depricated pydantic versions
            - Enabled WebsiteSearchTool() in research_analyst_agent in crew.py 
            - created llm model for openai insted of huggingface llm model 
            - .schema() depricated method changed to .model_json_schema() 
            - changed wrong , incomplete imports 
            - changed with open(file_path) as f to with open(file_path, encoding="utf-8") as f: (Added encoding to read file in utils for UnicodeDecodeError)
